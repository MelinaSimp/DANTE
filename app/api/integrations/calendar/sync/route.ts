// /api/integrations/calendar/sync — same shape as the gmail one.

import { NextRequest, NextResponse } from "next/server";
import { createServerSupabase } from "@/lib/supabase/server";
import { supabaseAdmin } from "@/lib/supabase/admin";
import { syncGoogleCalendar } from "@/lib/integrations/calendar/sync";
...
export async function POST() {
  ...
  try {
    const result = await syncGoogleCalendar({ workspaceId: profile.workspace_id, userId: user.id });
...
export async function GET(req: NextRequest) {
  ...
  for (const c of (creds || []) as Array<{ workspace_id: string; user_id: string }>) {
    try {
      const result = await syncGoogleCalendar({ workspaceId: c.workspace_id, userId: c.user_id });
