...
import { syncOutlook, distillEmailsIntoMemory } from "@/lib/integrations/outlook/sync";
...
export async function POST() {
  ...
  try {
    const sync = await syncOutlook({ workspaceId: profile.workspace_id, userId: user.id });
...
export async function GET(req: NextRequest) {
  ...
  for (const c of (creds || []) as Array<{ workspace_id: string; user_id: string }>) {
    try {
      const sync = await syncOutlook({ workspaceId: c.workspace_id, userId: c.user_id });
