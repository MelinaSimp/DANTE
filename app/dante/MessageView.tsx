...
export function UserMessage({ content }: { content: string }) {
  return (
    <div className="w-full flex justify-end">
      <div className="max-w-[80%] bg-[var(--glass-hover)] rounded-xl px-4 py-3">
  ...
  return (
    <div>
      ...
      <div className="mt-4 flex items-center gap-3 text-xs text-[var(--ink-subtle)] flex-wrap">
        <button onClick={onCopy} className="p-1.5 rounded text-[var(--ink-subtle)] hover:text-[var(--ink-muted)] hover:bg-[var(--glass-hover)]">
        ...
        <div className="relative">
          ...
          {rewriteOpen && (
            <div className="absolute left-0 top-full mt-1 z-10 rounded-lg border border-[var(--glass-border)] bg-[var(--canvas)] shadow-lg p-1 min-w-[160px]">
              {REWRITE_PRESETS.map((p) => (
                <button
                  ...
                  className="block w-full text-left rounded-md px-2 py-1.5 text-xs text-[var(--ink-muted)] hover:bg-[var(--glass-hover)]"
                >
                  {p.label}
                </button>
              ))}
          ...
          <div className="space-y-1">
            {followups.map((q, i) => (
              <button
                ...
                className="w-full text-left flex items-start gap-2 px-2 py-1.5 -mx-2 rounded-md text-sm text-[var(--ink-muted)] hover:bg-[var(--glass-hover)] transition"
              >
                <ArrowRight
...
export function SourcesBlock({ trace }: { trace: unknown }) {
  ...
  return (
    <div className="mt-4">
      ...
      {open && (
        <div className="mt-2 rounded-lg border border-[var(--glass-border)] bg-[var(--canvas-subtle)] p-3 space-y-3">
          {vault.length > 0 && (
            <div>
...
export function LiveThinking({
  ...
  return (
    <div className="rounded-lg border border-[var(--glass-border)] bg-[var(--canvas-subtle)]/80">
      <button
        onClick={() => setOpen((v) => !v)}
      ...
      >
        ...
        {deep && (
          <span className="inline-flex items-center gap-1 rounded-full bg-[var(--glass-hover)] px-2 py-0.5 text-[10px] text-[var(--ink-muted)]">
