...
export default function PlannerPanel({ agentId }: { agentId: string }) {
  ...
  return (
    <div className="flex h-full">
      {/* Chat list */}
      <div className="w-56 border-r border-[var(--glass-border)] bg-[var(--canvas-subtle)]/50 flex-col shrink-0 hidden md:flex">
        ...
        <div className="flex-1 overflow-y-auto p-2 space-y-0.5">
          {loadingChats ? (
            <div className="text-center py-8 text-[var(--ink-subtle)] text-xs">Loading...</div>
          ) : chats.length === 0 ? (
            <div className="text-center py-8 text-[var(--ink-subtle)] text-xs">No chats yet</div>
          ) : chats.map(c => (
            <div key={c.id} onClick={() => setCurrentChatId(c.id)}
              className={`group flex items-center justify-between px-3 py-2 rounded-lg cursor-pointer transition ${currentChatId === c.id ? "bg-[var(--canvas)] shadow-sm" : "hover:bg-[var(--canvas)]/50"}`}>
              <div className="flex items-center gap-2 min-w-0">
                <MessageSquare className="w-3.5 h-3.5 text-[var(--ink-subtle)] shrink-0" />
                <span className="text-sm text-[var(--ink-muted)] truncate">{c.title || "Untitled"}</span>
              </div>
        ...
        {!currentChatId && !loadingChats ? (
          <div className="flex-1 flex items-center justify-center">
            <div className="text-center">
              <MessageSquare className="w-12 h-12 text-[var(--ink-subtle)] mx-auto mb-3" />
              <p className="text-[var(--ink-subtle)] text-sm mb-4">Start a conversation with the meeting planner</p>
          ...
          <>
            ...
            <div className="flex-1 overflow-y-auto px-6 py-4">
              {loadingMsgs ? (
                <div className="text-center py-8 text-[var(--ink-subtle)] text-sm">Loading messages...</div>
              ) : messages.length === 0 ? (
                <div className="text-center py-16 text-[var(--ink-subtle)] text-sm">Send a message to start planning</div>
              ) : (
                <div className="max-w-3xl mx-auto space-y-4">
                  {messages.map((msg, i) => (
                    <div key={i} className={`flex ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
                      <div className={`max-w-[80%] px-4 py-3 rounded-2xl ${msg.role === "user" ? "bg-cyan-600 text-white" : "bg-[var(--glass-hover)] text-[var(--ink)]"}`}>
                  ...
                  {loading && (
                    <div className="flex justify-start">
                      <div className="bg-[var(--glass-hover)] rounded-2xl px-4 py-3 flex items-center gap-2">
                        <Loader2 className="w-4 h-4 animate-spin text-[var(--ink-subtle)]" />
                        <span className="text-sm text-[var(--ink-subtle)]">Thinking...</span>
                      </div>
                    </div>
                  )}
                  <div ref={endRef} />
            ...
            {/* Input */}
            <form onSubmit={sendMessage} className="border-t border-[var(--glass-border)] px-6 py-4">
              <div className="max-w-3xl mx-auto flex gap-3">
                <textarea ref={inputRef} value={input} onChange={e => setInput(e.target.value)} rows={1}
                  ...
                  className="flex-1 rounded-xl border border-[var(--glass-border)] bg-[var(--canvas-subtle)] px-4 py-3 text-sm resize-none focus:outline-none focus:ring-2 focus:ring-cyan-500/20" />
