...
import { Loader2, AlertTriangle, Save, X, Check } from "lucide-react";
...
function UsageBar({ used, limit }: { used: number; limit: number }) {
  ...
  const color =
    pct > 100
      ...
      : "bg-[var(--accent)]";
  return (
    <div className="h-1 w-full rounded-full bg-[var(--canvas-muted)] overflow-hidden">
      <div
...
function StatCard({ label, value, danger }: { label: string; value: string; danger?: boolean }) {
  return (
    <div className="card-flat px-4 py-3">
      ...
      <div
        className={`mt-1 text-lg font-semibold mono ${
...
function MeterCell({
  ...
  return (
    <td className="px-4 py-3 min-w-[140px]">
      <div className="mono text-[var(--ink)] text-xs mb-1">
        {formatNumber(used)} <span className="text-[var(--ink-subtle)]">/</span>{" "}
        {editing ? (
          <input
            type="number"
            className="w-20 px-1.5 py-0.5 rounded-[4px] bg-[var(--canvas)] border border-[var(--rule)] text-[var(--ink)] text-xs focus:border-[var(--accent)] focus:outline-none"
            value={draftValue ?? limit}
            onChange={(e) => onChange(Number(e.target.value))}
          />
        ) : (
          <span className="text-[var(--ink-muted)]">{formatNumber(limit)}</span>
        )}
      </div>
...
export default function UsageAPanel() {
  ...
  if (!rows) {
    return <div className="p-4 text-[var(--ink-muted)]">Failed to load usage data.</div>;
  }
  ...
  return (
    <div>
      ...
      {totals && (
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 mb-6">
          ...
          <StatCard
            ...
            danger={totals.overage_cents > 0}
          />
        </div>
      )}

      {totals && totals.over_count > 0 && (
        <div className="mb-6 rounded-[6px] border border-[var(--danger)]/30 bg-[var(--danger-soft)] p-4 flex items-start gap-3">
          <AlertTriangle
            className="h-5 w-5 text-[var(--danger)] mt-0.5 shrink-0"
            strokeWidth={1.5}
          />
          <div className="text-sm">
            ...
            <p className="text-[var(--ink-muted)] text-xs mt-1">
              Total overage billable: <span className="mono">{formatCost(totals.overage_cents)}</span>
            </p>
          </div>
        </div>
      )}
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-[var(--canvas-subtle)] border-b border-[var(--rule)]">
              <tr>
                <th className="label-section text-left px-4 py-3">Workspace</th>
                <th className="label-section text-left px-4 py-3">Plan</th>
                <th className="label-section text-left px-4 py-3">LLM tokens</th>
                <th className="label-section text-left px-4 py-3">Emails</th>
                <th className="label-section text-left px-4 py-3">SMS</th>
                <th className="label-section text-left px-4 py-3">Voice min.</th>
                <th className="label-section text-left px-4 py-3">Cost</th>
                <th className="label-section text-left px-4 py-3">Overage</th>
                <th className="label-section text-right px-4 py-3"></th>
              </tr>
            </thead>
            <tbody>
              {rows.length === 0 && (
                <tr>
                  <td colSpan={9} className="px-4 py-12 text-center text-[var(--ink-subtle)]">
                    No workspaces yet.
                  </td>
                </tr>
              )}
              {rows.map((r) => {
                ...
                return (
                  <tr
                    ...
                    className={`border-t border-[var(--rule)] hover:bg-[var(--canvas-subtle)] ${
                      r.overage.any_over ? "bg-[var(--danger-soft)]" : ""
                    }`}
                  >
                    <td className="px-4 py-3 text-[var(--ink)] font-medium">
                      {r.usage.workspace_name}
                      {r.quota.hard_cap && (
                        <span className="ml-2 mono text-[10px] text-[var(--flag)] bg-[var(--flag-soft)] px-1.5 py-0.5 rounded">
                          HARD CAP
                        </span>
                      )}
                    </td>
                    <td className="px-4 py-3">
                      {isEditing ? (
                        <input
                          className="w-24 px-2 py-1 rounded-[4px] bg-[var(--canvas)] border border-[var(--rule)] text-[var(--ink)] text-xs focus:border-[var(--accent)] focus:outline-none"
                          value={draft.plan_name ?? ""}
                          onChange={(e) => setDraft({ ...draft, plan_name: e.target.value })}
                        />
                      ) : (
                        <span className="text-[var(--ink-muted)] capitalize">{r.quota.plan_name}</span>
                      )}
                    </td>
                    <MeterCell
                    ...
                    />
                    <td className="px-4 py-3 mono text-[var(--ink-muted)]">
                      {formatCost(r.usage.total_cost_cents)}
                    </td>
                    <td className="px-4 py-3">
                      {r.overage.any_over ? (
                        <span className="mono text-[var(--danger)] font-medium">
                          {formatCost(r.overage.overage_cents)}
                        </span>
                      ) : (
                      ...
                      {isEditing ? (
                        <div className="flex items-center gap-2 justify-end">
                          ...
                          <button
                            onClick={saveEdit}
                            disabled={saving}
                            className="p-1.5 rounded-[4px] bg-[var(--verified-soft)] text-[var(--verified)] hover:brightness-95 disabled:opacity-40"
                          >
                            {saving ? (
                          ...
                          <button
                            ...
                            className="p-1.5 rounded-[4px] bg-[var(--canvas-subtle)] text-[var(--ink-muted)] hover:bg-[var(--canvas-muted)]"
                          >
      ...
      <p className="text-[11px] text-[var(--ink-subtle)] mt-4">
        Counters reset on the 1st of each month (UTC). Overage is calculated at the configured rate
        per unit above each quota. Set{" "}
        <code className="mono bg-[var(--canvas-subtle)] px-1.5 py-0.5 rounded text-[var(--ink-muted)]">
          Hard cap
        </code>{" "}
        to block further billable actions once a workspace exceeds its plan.
      </p>
