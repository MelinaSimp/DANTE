# All SQL Files - Copy-Paste Ready
...
WHERE table_name = 'agents' 
  AND column_name IN ('agent_role', 'is_specialist', 'parent_agent_id', 'routing_keywords');

-- Check steps table has new columns
SELECT column_name, data_type 
FROM information_schema.columns 
WHERE table_name = 'steps' 
  AND column_name IN ('selected_data_source_ids', 'loop_config', 'transfer_config', 'sms_config');

-- Check new tables exist
SELECT table_name 
FROM information_schema.tables 
WHERE table_name IN ('scheduled_sms', 'integration_credentials', 'response_cache', 'error_logs');

-- Check step type constraint
SELECT constraint_name, check_clause 
FROM information_schema.check_constraints 
WHERE constraint_name = 'steps_type_check';
```
...
-- Only use if you need to completely rollback

-- Drop new tables
DROP TABLE IF EXISTS error_logs CASCADE;
DROP TABLE IF EXISTS response_cache CASCADE;
DROP TABLE IF EXISTS integration_credentials CASCADE;
DROP TABLE IF EXISTS scheduled_sms CASCADE;

-- Remove columns from conversations
ALTER TABLE conversations 
  DROP COLUMN IF EXISTS transferred_from_agent_id,
  DROP COLUMN IF EXISTS transferred_to_agent_id,
  DROP COLUMN IF EXISTS transfer_history,
  DROP COLUMN IF EXISTS parent_conversation_id,
  DROP COLUMN IF EXISTS loop_state;

-- Remove columns from agent_data_sources
ALTER TABLE agent_data_sources 
...
-- Remove columns from steps
ALTER TABLE steps 
  DROP COLUMN IF EXISTS selected_data_source_ids,
  DROP COLUMN IF EXISTS loop_config,
  DROP COLUMN IF EXISTS transfer_config,
  DROP COLUMN IF EXISTS sms_config;

-- Restore old step type constraint (if needed)
ALTER TABLE steps DROP CONSTRAINT IF EXISTS steps_type_check;
ALTER TABLE steps ADD CONSTRAINT steps_type_check 
...
ALTER TABLE agents 
  ...
  DROP COLUMN IF EXISTS is_specialist,
  DROP COLUMN IF EXISTS parent_agent_id,
  DROP COLUMN IF EXISTS routing_keywords;

-- Drop indexes
DROP INDEX IF EXISTS idx_agents_specialist;
DROP INDEX IF EXISTS idx_data_sources_integration;
DROP INDEX IF EXISTS idx_conversations_transfer;
```

## Notes

1. **Run the main migration first** - This is the primary migration file
2. **Verify with queries** - Use the verification queries to ensure everything was created correctly
3. **Test in staging first** - Always test migrations in a staging environment before production
